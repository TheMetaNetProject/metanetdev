...
public class CodeGenerationHelper {
    ...
    public CodeGenerationHelper(OWLOntology owlOntology, OWLNamedIndividual individual, CodeGenerationInference inf) {
    ...
    public <X> Collection<X> getPropertyValues(OWLNamedIndividual i, OWLObjectProperty p, Class<X> c) {
    	try {
    		Constructor<X> constructor = c.getConstructor(OWLOntology.class, IRI.class, CodeGenerationInference.class);
    		Set<X> results = new HashSet<X>();
    		for (OWLIndividual j : inference.getObjectPropertyValues(i, p)) {
    			if (!j.isAnonymous()) {
    				results.add(constructor.newInstance(owlOntology, j.asOWLNamedIndividual().getIRI(), inference));
    			}
    		}
    ...
    public <X> Collection<X> getPropertyValues(OWLNamedIndividual i, OWLDataProperty p, Class<X> c) {
    	Set<X> results = new HashSet<X>();
    	for (OWLLiteral l : inference.getDataPropertyValues(i, p)) {
