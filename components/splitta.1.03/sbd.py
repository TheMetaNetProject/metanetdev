...
This version includes modifications by Jason Bolton <jebolton@icsi.berkeley.edu>
    ...
    def segment(self, use_preds=False, tokenize=False, output=None, list_only=False):
        ...
        while frag:
            ...
            if frag.ends_seg or (use_preds and frag.pred>thresh) or (not use_preds and frag.label>thresh):
                ...
                # JASON EDIT: changed sentence delimiter to make more clear
                else: spacer = '---\n'
                if output: output.write(spacer + sent_text + "\n")
...
if __name__ == '__main__':
    ...
    if len(args) > 0:
        options.test = args[0]
        # JASON EDIT: it's ok if argument is a dir
    ...
    ## JASON EDIT: check if path is a directory do a loop for each file, based on expected input structure
    if os.path.isdir(options.test):
        doc_files = os.listdir(options.test)
        counter = 0
        for doc_file in doc_files:
            if doc_file[-9:] == "input.txt":
                print "num processed: "+str(counter)
                counter += 1
                print "Processing: "+doc_file
                file_text = file(options.test+"/"+doc_file).read()
                if not file_text:
                    continue
                test = get_data(options.test+"/"+doc_file, tokenize=True)
                test.featurize(model, verbose=True)
                model.classify(test, verbose=True)
                output_file = file(options.test+"/"+doc_file.split("_")[0]+"_output.txt","a")
                test.segment(use_preds=True, tokenize=options.tokenize, output=output_file)
    else:
