...
class SesameTripleStore(BaseBackend):
    ...
    def __init__(self):
        super(SesameTripleStore, self).__init__()
        self._nsmap = {}
    ...
    def contexts(self):
        ...
        return [c['contextID']['value'] for c in
    ...
    def add_turtle(self, data, context):
        data = self._get_file(data)
        self._add(data, 'turtle', self._get_context(context))
    ...
    def pselect(self, qid, bindings={}):
        if bindings:
            ...
            qStr = u'%s\n%s\nBINDINGS %s { ( %s ) }' % (self._query_header,
                                                        self._pqueries[qid],
                                                        u' '.join(keylist), u' '.join(vallist))
        else:
            qStr = u'%s\n%s' % (self._query_header,self._pqueries[qid])
        return self.select(qStr)
