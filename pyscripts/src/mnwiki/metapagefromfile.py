...
#coding: utf-8
"""
This bot is a copy of pagefromfile.py that has been customized to
take care of a few details for the purpose of importing linguistic
metaphor pages.  Specifically, it allows for the merger of examples
if the page already exists and contains examples not present in
the content to be uploaded.  Similarly, it allows for the merging
of tags.

It takes its input from a file that contains a number of
...
-mergetemplates for liguistic metaphor pages: merge the templates
"""
#
# (C) Andre Engels, 2004
...
class PageFromFileRobot:
    ...
    def __init__(self, reader, force, append, mergetemplates, summary, minor, autosummary,
                 dry, nocontents):
        self.reader = reader
        self.force = force
        self.append = append
    ...
    def dotemplatemerger(self, existing, contents):
        '''
        Looks up the metaphor page and preserves examples that are in the wiki, but
        not in the contents being imported.  Also merges Tags.  Everything else is
        overwritten with content from the new import.
        '''
        elines = existing.splitlines()
        clines = contents.splitlines()

        # do tags first: bc easier
        # - search for Tag= in the template, if it's there, copy over to contents
        etags = []
        for line in elines:
            tm = re.search('^\|Tag=(.*)$',line,re.UNICODE)
            if tm:
                etags = tm.group(1).strip().split(',')
                break
        if etags:
            for i in range(len(clines)):
                tm = re.search('^\|Tag=(.*)$',clines[i],re.UNICODE)
                if tm:
                    ctags = set(tm.group(1).strip().split(','))
                    for tag in etags:
                        ctags.add(tag.strip())
                    clines[i]="|Tag="+",".join(ctags)
                    
        # do examples: bc harder
        # start from the beginning (use elines)
        e = 0
        numelines = len(elines)
        eHasExamples = False
        # advance existing to the Examples line
        while e < numelines:
            line = elines[e].strip()
            if line == '|Examples={{Example':
                eHasExamples = True
                break
            e += 1
        # if existing page doesn't have examples, overwrite it
        if eHasExamples == False:
            return contents

        # note: at this point elines[e] is where examples start |Examples={{Example
        # start using clines here
        olines = list()
        c = 0
        numclines = len(clines)
        while c < numclines:
            line = clines[c].strip()
        ...
        # note: at this going clines[c] is either |Examples= or with |Examples={{Example
        cline = clines[c].strip()
        if (cline == "|Examples="):
            # contents has no examples
            # then write in all examples from existing
            while e < numelines:
                eline = elines[e].strip()
                if eline.startswith('|Comments='):
                    break
                olines.append(eline)
                e += 1
            c += 1
            # to skip "|Examples=" in clines
        elif (cline == "|Examples={{Example"):
            #print all c examples
            cexamples = list()
            while c < numclines:
                cline = clines[c].strip()
                if (cline == '}}'):
                    break
                olines.append(cline)
                if (cline.startswith('|Example.Text=')):
                    cexamples.append(cline);
                c += 1
            #now go through examples
            cexset = set(cexamples)
            e += 1
            c += 1
            # is is to skip e's |Examples={{Example and c's }}
            olines.append('}}{{Example');
            printlines = True
            while e < numelines:
                eline = elines[e].strip()
                if (eline.startswith('|Example.Text=')):
                    if (eline in cexset):
                        printlines = False
                    else:
                        printlines = True
    ...
    def put(self, title, contents):
        ...
        if page.exists():
            ...
            elif self.mergetemplates:
                pywikibot.output(u"Page %s already exists, ***merging templates!"
                                 % title)
                excontent = page.get() + "\n"
                if (excontent == contents):
                    return
                contents = self.dotemplatemerger(excontent, contents)
                comment = u'merged with existing - '+comment
            else:
...
def main():
    ...
    reader = PageFromFileReader(filename, pageStartMarker, pageEndMarker,
                                ...
                                notitle)
    bot = PageFromFileRobot(reader, force, append, mergetemplates, summary, minor, autosummary,
